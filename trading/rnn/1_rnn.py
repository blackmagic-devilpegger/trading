...
class CryptoRNN:
    def __init__(self, api_key, seq_length=10):
        ...
        """
        # Initialisieurung der Kraken-API
        self.api = krakenex.API()
        self.api.key = api_key                                                      # API-Schlüssel für die Authentifizierung
        self.seq_length = seq_length                                                # Anzahl der Zeitpunkte in einer Eingabesequenz
        self.df = None                                                              # Speichert historische Daten
        self.X = None                                                               # Eingabedaten für das Modell
        self.y = None                                                               # Zielwerte für das modell
        self.model = None                                                           # RNN-Modell
    ...
    def fetch_data(self, pair="XXBTZUSD", interval=60, days=30):
        ...
        if len(data['error']) == 0:
            # Konventiere die Daten in einen DataFrame
            ohlc = data['result'][pair]
            self.df = pd.DataFrame(
                ohlc, columns=['time', 'open', 'high', 'low', 'close', 'vwap', 'volume', 'count']
            )
            self.df['time'] = pd.to_datetime(self.df['time'], unit='s')              # Zeitspalte konvertieren
            ...
            print(self.df.head())                                                    # Zeige die ersten Zeilen der Daten
        else:
            raise Exception(f"API-Fehler: {data['error']}")                          # Fehlermeldung bei API-Fehler
    ...
    def prepare_data(self, validation_split=0.2):
        ...
        # Sequenzen(X) und Zielwerte(y) erstellen
        X, y = [], []
        ...
        # Konvertiere Sequenzen und Zielwerte  in Tensoren
    ...
    def create_model(self, input_size, hidden_size, output_size, num_layers=1):
        ...
        # Definiert ein einfaches RNN-Modell
        class RNNModel(nn.Module):
            def __init__(self, input_size, hidden_size, output_size):
                ...
                self.rnn = nn.RNN(input_size, hidden_size, num_layers=1, batch_first=True)        # RNN-Layer(Schicht)
                self.fc = nn.Linear(hidden_size, output_size)                                     # Fully Connected Layer(Lineare Schicht für die Ausgabe)
...
if __name__ == "__main__":
    ...
    test_input = crypto_rnn.X_val[:5]                                                     # Erste 5 Sequenzen aus den Validierungsdaten
    predictions = crypto_rnn.predict(test_input)                                          # Vorhersagen mit dem trainierten Modell
    print("Vorhersagen:", predictions)

    # Rücknormalisierung der Vorhersagen auf den Ursprünglichen Maßstab
    # Mittelwert und Standardabweichung der Originaldaten(Ursprüngliche) berechnen
    ...
    predictions_original_scale = predictions_original_scale.cpu().detach().numpy()  # Konvertierung in numpy-Array
    ...
    mae_predictions = np.mean(np.abs(predictions_original_scale - actual_prices))
    mse_predictions = np.mean((predictions_original_scale - actual_prices) ** 2)

    print(f"Mean Absolute Deviation (MAD): {mad_predictions:.4f} USD")
    print(f"Mean Absolute Error (MAE): {mae_predictions:.4f} USD")
    print(f"Mean Squared Error (MSE): {mse_predictions:.4f} USD")

    # Visualisierung der Vorhersagen
    import matplotlib.pyplot as plt

    plt.figure(figsize=(12, 6))
    plt.plot(actual_prices, label="Actual Prices", color='blue', alpha=0.7)                         # Tatsächliche Preise
    plt.plot(predictions_original_scale, label="Predicted Prices", color='red', linestyle='--', alpha=0.7)    # Vorhergesagte Preise
    plt.title("Bitcoin Price Prediction")                                                                 # Titel des Plots
    plt.xlabel("Time Steps")                                                                              # Beschriftung der x-Achse
    plt.ylabel("Price (USD)")                                                                             # Beschriftung der y-Achse
    plt.legend()                                                                                          # Anzeige der Legende
    plt.show()                                                                                            # DArstellung des plots

    # Handelsstrategien
    # Einfache Preisprognose basierend auf dem aktuellen und vorhergesagten Preis
    current_price = crypto_rnn.df.iloc[-1]['close']                                                 # Aktueller Schlusskurs aus  den Daten
    predicted_price = predictions_original_scale[0].item()                                          # Vorhergesagter nächster Schlusskurs

    # Entscheidung basierend auf der Vorhersage
