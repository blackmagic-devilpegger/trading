...
# EXPERIMENT
# LEARNING-RATE angepasst  = zuvor: 0.001   ,jetzt: 0.0001 standardmäßig, 0.0005 im Hauptcode (niedrigere lernrate)
#Erwartung: Stabilere Anpassung, Overfitting minimieren
#Ergebnis: Stabilerer Trainingsverlauf

# EARLY-STOPPING (patience) angepasst = zuvor: patience: 5  ,jetzt: 10
#Erwartung: Bessere Validierungsverluste durch längeres Training vor Abbruch
#Ergebnis:  Training verlief stabil mit einer stetigen Abnahme des Validierungsverlustes über die Epochen.

# +SICHERHEITSMARGE hinzugefügt von 2% (um Fehlsignale zu reduzieren)
#Erwartung: Stabilere und sinnvollere Handelsempfehlungen , reduzierung von Fehlsignalen
#Ergebnis: Modell reduziert ungenaue Kaufentscheidungen, durch die Marge werden die Handelsstrategien robuster /weniegr anfällig ggü Schwankungen

# +MAD (zu Vorhersagen & Durchschnittspreise) Berechnung hinzugefügt
#Erwartung: Quantitative bewertung der Modellgüte, Vgl. mit Durchschnittspreis soll zeigen ob das Modell signifikant besser ist.
#Ergebnis: MAD (of Prediction) Vorhersagen war höher als das MAD für den Durchschnittspreis: (5452 USD zu 3010 USD)
#          Zeigt, dass das Modell Schwankungen erfasst, aber potenziell weiter optimiert werden kann.

# +KONVERTIERUNG DER VORHERSAGEN hinzugefügt (notwendig für MAD-berechnung)
#Erwartung: Korrekte Rücknormalisireung für Vergleich und Visualisierung.
#Ergebnis: Technisch korrekt umgesetzt, aber die schlechtere Modellleistung blieb offensichtlich.

# => Valid_loss und Train-loss sinken kontinuierlich ohne zwischendurch zu steigen.
...
if __name__ == "__main__":
    # Instanziiere die Klasse mit meinem API-Key
    crypto_rnn = CryptoRNN(api_key='w3if4ZjPEKdgCVsj7J/KVRgkSKhAhYBcJJrrp8gXTfrRdlylAVafK85F')

    # 1. Daten abrufen
    crypto_rnn.fetch_data()                 # Historische Daten von der Kraken-API abrufen

    # 2. Daten vorbereiten
    crypto_rnn.prepare_data()               # Daten normalisieren und in Trainings- und Validierungsdaten aufteilen

    # 3. Modell erstellen
    crypto_rnn.create_model(input_size=4, hidden_size=50, output_size=1)    # RNN-Modell initialisieren
    ...
    train_loss_values = crypto_rnn.train_loss_values            # Trainingsverluste über die Epochen
    val_loss_values = crypto_rnn.val_loss_values                # Validierungsverluste über die Epochen
    ...
    test_input = crypto_rnn.X_val[:5]                           # Verwendet die ersten 5 Sequenzen aus den Validierungsdaten
    predictions = crypto_rnn.predict(test_input)                # Vorhersagen mit dem trainierten Modell
    ...
    # Rücknormalisieren der Vorhersagen auf den Ursprünglichen Maßstab
    ...
    actual_prices = crypto_rnn.df['close'].iloc[-(len(crypto_rnn.y_val)):].values[:len(predictions_original_scale)]
    ...
    # Mean Absolute Deviation for predictions (MAD) für Vorhersagen berechnen
    ...
    # Mean Absolute Deviation for average price (MAD) für Durchschnittspreis berechnen
    ...
    # Ausgabe der zusätzlichen Metriken
    print(f"Mean Squared Error (MSE): {mse:.4f} USD")
    print(f"Mean Absolute Error (MAE): {mae:.4f} USD")

    # Anzahl der Datenpunkte, die visualisiert werden (maximal 50)
    n = min(50, len(predictions_original_scale))

    # Bereite tatsächliche Werte (Validierungsdaten) und Vorhersagen für die Visualisierung vor
    ...
    # Visualisierung der  tatsächlichen Preise und Vorhersagen
    import matplotlib.pyplot as plt
    ...
    # Handelsstrategien
    # Einfache Preisprognose basierend auf der Sicherheitsmarge von 2%
    ...
    # Entscheidungen basierend auf der Sicherheitsmarge
